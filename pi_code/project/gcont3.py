...
def send_control_signals(steering_angle, throttle):
    ...
    throttle_pwm = int(map_range(throttle, -32768, 32767, 1000, 2000))
    
    # Send control signals to Arduino Nano
    ser.write(f"{servo_pwm},{throttle_pwm}\n".encode())

# Main loop to read controller inputs, send control signals, and read voltage signal
...
for event in controller_dev.read_loop():
    ...
    # Read voltage signal from Arduino Nano's A0 pin
    # Note: Make sure the Arduino is sending the voltage value correctly via serial
    # You may need to modify the Arduino code to send the voltage value over serial
    ser.write(b"read_voltage\n")
    voltage_signal = ser.readline().strip()
    
    # Print voltage signal at 1-second interval
    if time.time() - last_print_time >= 1:
        try:
            print("Voltage Signal:", voltage_signal.decode('ascii'))
        except UnicodeDecodeError:
            print("Error decoding voltage signal")
        last_print_time = time.time()
