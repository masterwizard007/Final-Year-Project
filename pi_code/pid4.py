...
target_rpm = 2000  # Target RPM for the PID controller
...
def send_control_signals(ser, steering_angle, throttle):
    ...
    throttle_pwm = int(map_range(throttle, -32768, 32767, 1000, 2000))  # Map throttle to 1000-2000 range
...
def print_output(rpm, voltage_value, pitch, yaw, roll):
    ...
    print_debug(f"RPM: {rpm}, Voltage: {voltage_value}, Pitch: {pitch}, Yaw: {yaw}, Roll: {roll}")
...
def pid_control(throttle, current_rpm):
    ...
    return throttle - output  # Subtracting output to decrease throttle

# PID control task
def pid_control_task(ser):
    global rpm
    try:
        while True:
            throttle = pid_control(500, rpm)  # Initial throttle value 1500
            send_control_signals(ser, 0, throttle)  # Steering angle set to 0
            time.sleep(0.1)  # Adjust sleep time if necessary
    except Exception as e:
        print("Exception in PID control task:", e)
...
def main():
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future = executor.submit(pid_control_task, ser)
        ...
        # Main loop to read IR sensor data and send control signals
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            future1 = executor.submit(read_ir_sensor_rpm)
            future2 = executor.submit(read_ir_sensor_voltage, ser)

            for steering_angle in read_joystick_inputs(controller_dev):
                send_control_signals(ser, steering_angle, 0)  # Only update steering, throttle controlled by PID
