...
# Print output
def print_output(rpm, throttle, pitch, yaw, roll):
    print("\033[H\033[J")  # Clear the terminal
    print(f"RPM : {Fore.GREEN}{rpm:.4f}{Style.RESET_ALL}")
    print(f"Throttle : {Fore.CYAN}{throttle:.4f}{Style.RESET_ALL}")
    print(f"Pitch : {Fore.YELLOW}{pitch:.4f}{Style.RESET_ALL}")
    print(f"Yaw : {Fore.MAGENTA}{yaw:.4f}{Style.RESET_ALL}")
    print(f"Roll : {Fore.BLUE}{roll:.4f}{Style.RESET_ALL}")
...
# Read gyro values from MPU6050 and calculate pitch, yaw, and roll angles
def read_gyro_values():
    ...
    pitch = math.degrees(math.atan2(gyro_y, math.sqrt(gyro_x**2 + gyro_z**2)))
    yaw = math.degrees(math.atan2(gyro_x, math.sqrt(gyro_y**2 + gyro_z**2)))
    roll = math.degrees(math.atan2(gyro_z, math.sqrt(gyro_x**2 + gyro_y**2)))

    return pitch, yaw, roll

# Save data to SQLite database
def save_to_database(rpm, throttle, pitch, yaw, roll):
    conn = sqlite3.connect('data.db')
    cursor = conn.cursor()
    timestamp = int(time.time())
    cursor.execute('''INSERT INTO SensorData (timestamp, rpm, throttle, pitch, yaw, roll)
                      VALUES (?, ?, ?, ?, ?, ?)''',
                   (timestamp, rpm, throttle, pitch, yaw, roll))
    conn.commit()
    conn.close()
...
def main():
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            future1 = executor.submit(read_ir_sensor_rpm)
            future2 = executor.submit(read_ir_sensor_voltage, ser)

            for steering_angle, throttle in read_joystick_inputs(controller_dev):
                send_control_signals(ser, steering_angle, throttle)
                pitch, yaw, roll = read_gyro_values()
                print_output(rpm, throttle, pitch, yaw, roll)
                save_to_database(rpm, throttle, pitch, yaw, roll)
