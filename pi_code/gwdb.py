...
# Print output
def print_output(rpm, voltage_value, gyro_values):
    ...
    print(f"Gyro Values: {Fore.BLUE}X:{gyro_values['x']}, Y:{gyro_values['y']}, Z:{gyro_values['z']}{Style.RESET_ALL}")
...
def read_ir_sensor_voltage(ser):
    try:
        while True:
            ...
            gyro_values = read_gyro_values()
            print_output(rpm, voltage_value, gyro_values)
            save_to_database(rpm, voltage_value, gyro_values)
    except Exception as e:
...
def save_to_database(rpm, voltage_value, gyro_values):
    conn = sqlite3.connect('data.db')
    cursor = conn.cursor()
    timestamp = int(time.time())
    cursor.execute('''INSERT INTO SensorData (timestamp, rpm, voltage, gyro_x, gyro_y, gyro_z)
                      VALUES (?, ?, ?, ?, ?, ?)''',
                   (timestamp, rpm, voltage_value, gyro_values['x'], gyro_values['y'], gyro_values['z']))
    conn.commit()
    conn.close()
